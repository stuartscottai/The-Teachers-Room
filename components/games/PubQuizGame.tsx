...
import { ArrowLeft, Clock, ArrowRight, RotateCcw, CheckCircle, XCircle, Plus, Minus, List, Trophy, RefreshCw, Play, Check, Edit2, Volume2, VolumeX, Maximize2, Minimize2, AlertTriangle, Star } from 'lucide-react';
...
const AnimatedScore: React.FC<{ score: number }> = ({ score }) => {
    ...
    return (
        <div className="relative">
            <div className="text-5xl font-black font-mono leading-none tracking-tight transition-all text-slate-900">
                {displayScore}
            </div>
            {diff !== 0 && (
    ...
    return (
        <div ref={containerRef} className={`min-h-screen bg-slate-800 flex flex-col ${isFullscreen ? 'h-screen' : 'h-[calc(100vh-4rem)]'} overflow-hidden relative transition-colors duration-500`}>
            ...
            <div className="flex-grow flex flex-col items-center justify-center p-4 relative z-10 overflow-y-auto">
                ...
                {phase === 'review' && currentRound && (
                    <div className="w-full max-w-6xl h-full flex flex-col animate-fade-in">
                        <div className="text-center mb-6">
                            <h2 className="text-4xl font-display font-black text-white drop-shadow-md">Round Review: {currentRound.name}</h2>
                            <p className="text-white/60 font-bold text-lg">Review answers before scoring.</p>
                        </div>
                        
                        <div className="flex-1 overflow-y-auto bg-white rounded-3xl shadow-2xl border-4 border-slate-200 p-8 space-y-6">
                            {currentRound.questions.map((q, idx) => (
                                <div key={idx} className="border-b border-slate-100 pb-6 last:border-0">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className="bg-slate-900 text-white w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg flex-shrink-0 mr-4 shadow-sm">{idx + 1}</span>
                                        <p className="font-bold text-2xl text-slate-800 flex-1 leading-tight">{q.question}</p>
                                    </div>
                                    <div className="ml-14">
                                        {revealedReviewAnswers[idx] ? (
                                            <div className="text-green-600 font-bold text-3xl animate-fade-in flex items-start gap-3 mt-2">
                                                <CheckCircle size={32} className="mt-1 flex-shrink-0" /> {q.answer}
                                            </div>
                                        ) : (
                                            <button 
                                                onClick={() => {
                                                ...
                                                className="text-lg font-bold text-brand-blue hover:text-white hover:bg-brand-blue border-2 border-brand-blue px-4 py-2 rounded-lg transition-colors mt-2"
                                            >
                                                Reveal Answer
                                            </button>
                                        )}
                        ...
                        <div className="mt-6 flex justify-center py-4 gap-4">
                            <button 
                                onClick={() => {
                                    const allRevealed = Array(currentRound.questions.length).fill(true);
                                    setRevealedReviewAnswers(allRevealed);
                                }}
                                className="px-8 py-4 bg-white text-slate-700 rounded-xl font-bold text-xl hover:bg-slate-100 shadow-lg transition-transform hover:scale-105"
                            >
                                Reveal All
                            </button>
                            <button 
                                onClick={() => setPhase('scoring')}
                                className="px-10 py-4 bg-brand-yellow text-slate-900 rounded-xl font-bold text-xl hover:bg-yellow-300 shadow-lg transition-transform hover:scale-105"
                            >
                                Go to Scoring
                            </button>
                        </div>
                ...
                {/* SCORING PHASE */}
                {phase === 'scoring' && (
                    <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-8 animate-fade-in border-4 border-slate-200">
                        <h2 className="text-4xl font-display font-bold text-slate-800 text-center mb-2">Round Complete!</h2>
                        <p className="text-center text-slate-500 mb-8 text-lg font-medium">Enter points for this round.</p>
                        <div className="space-y-4">
                            {scores.map((score, i) => (
                                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="font-bold text-xl text-slate-700 w-1/3 truncate">{teamNames[i]}</div>
                                    <div className="font-mono font-bold text-3xl text-brand-blue w-1/3 text-center">{score}</div>
                                    <div className="flex items-center gap-2 w-1/3 justify-end">
                                        <button 
                                            onClick={() => handleScoreUpdate(i, -1)}
                                            className="w-12 h-12 flex items-center justify-center bg-white border-2 border-slate-200 rounded-lg hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-colors"
                                        >
                                            <Minus size={24} />
                                        </button>
                                        <button 
                                            onClick={() => handleScoreUpdate(i, 1)}
                                            className="w-12 h-12 flex items-center justify-center bg-white border-2 border-slate-200 rounded-lg hover:bg-green-50 hover:text-green-500 hover:border-green-200 transition-colors"
                                        >
                                            <Plus size={24} />
                                        </button>
                                    </div>
                        ...
                        <button 
                            onClick={finishRound}
                            className="w-full mt-8 py-4 bg-brand-blue text-white rounded-xl font-bold text-xl hover:bg-sky-600 transition-all shadow-md flex items-center justify-center"
                        >
                        ...
                        <div className={`relative w-full h-full transition-all duration-700 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                            ...
                            <div className={`absolute inset-0 [backface-visibility:hidden] rounded-2xl shadow-2xl overflow-hidden flex flex-col h-full bg-white ${isFlipped ? 'pointer-events-none' : ''}`}>
                                ...
                                <div className="bg-brand-blue text-white p-4 flex justify-between items-center h-24 flex-shrink-0 relative z-10">
                                    <div className="font-bold text-2xl opacity-90">{currentRound?.name}</div>
                                    <div className="bg-white/20 px-4 py-1 rounded-full font-black text-2xl">Q{currentQuestionIndex + 1}</div>
                                    <div className="font-bold text-xl opacity-80">{currentRound?.questions.length} Total</div>
                                </div>
                                ...
                                <div className={`h-24 flex items-center justify-between px-8 relative flex-shrink-0 z-50 transition-colors duration-300 ${isTimesUp ? 'bg-red-600' : 'bg-gradient-to-r from-brand-blue to-sky-500'}`}>
                                    ...
                                    <button 
                                        ...
                                        className="bg-white text-brand-blue px-10 py-3 rounded-full font-bold text-2xl shadow-lg hover:scale-105 transition-transform flex items-center relative z-50 border-2 border-white"
                                    >
                                        Reveal Answer
                                    </button>
                                    ...
                                    {options.timerSeconds > 0 && (
                                        <div className="text-white font-mono font-bold text-4xl opacity-90 flex items-center pointer-events-none absolute left-1/2 -translate-x-1/2">
                                            {isTimesUp ? (
                                                ...
                                                <><Clock size={32} className="mr-3" /> {timeLeft}</>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    <button 
                                        ...
                                        className="text-white font-bold text-xl hover:bg-white/20 px-4 py-2 rounded-lg transition-colors flex items-center opacity-90 hover:opacity-100 relative z-50"
                                    >
                                        Go to next question <ArrowRight size={24} className="ml-2" />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className={`absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl shadow-2xl overflow-hidden flex flex-col h-full bg-slate-50 ${!isFlipped ? 'pointer-events-none' : ''}`}>
                                ...
                                <div className="flex-grow flex flex-col items-center justify-center p-8 bg-white text-center overflow-hidden w-full relative z-0">
                                    ...
                                    <div className="mt-4 w-full bg-slate-50 rounded-2xl p-4 border-2 border-slate-200 flex-shrink-0 relative z-10">
                                        <h4 className="text-sm font-bold text-slate-400 uppercase mb-3 tracking-widest">Quick Score (+1 Point)</h4>
                                        <div className="flex flex-wrap justify-center gap-3">
                                            {scores.map((s, i) => (
                                                <button 
                                                    key={i}
                                                    onClick={(e) => { e.stopPropagation(); handleScoreUpdate(i, 1); }}
                                                    className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 font-bold hover:bg-green-50 hover:border-green-300 hover:text-green-600 transition-all shadow-sm active:scale-95 flex items-center"
                                                >
                                ...
                                <div className="h-24 flex flex-shrink-0 relative z-50">
                                    <button 
                                        onClick={handleNextQuestion}
                                        className="flex-1 bg-brand-blue text-white font-bold text-2xl hover:bg-sky-600 transition-colors flex items-center justify-center"
                                    >
                                        Go to next question <ArrowRight size={24} className="ml-2" />
                                    </button>
                                </div>
