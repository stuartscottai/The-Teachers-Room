...
export const PagesCanvas: React.FC<{
  ...
  return (
    <div
      className="ws-canvas flex-1 min-w-0 overflow-auto bg-slate-200"
      onMouseDown={() => onSelectElementId(null)}
    >
      <div className="ws-pages-wrap py-8 px-4 pb-36 flex flex-col items-center gap-6">
        {pages.map((p, idx) => (
          <div
            key={p.id}
        ...
        {onAddPage && (
          <button
            type="button"
            onClick={onAddPage}
            className="no-print w-[210mm] max-w-full py-3 rounded-2xl border-2 border-dashed border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-extrabold"
          >
            + Add Page
          </button>
        )}
